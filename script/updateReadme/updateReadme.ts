...
function getLastUpdateTime(
    filePath: string,
    getLastUpdateWay: 'git' | 'file' = 'file',
): number {
    // 如果指定使用文件修改时间，直接返回
...
class ScriptFileSystem {
    ...
    read(existingScriptInfo?: ScriptInfo): ScriptInfo {
        ...
        // 2. 读取顶级域名下的脚本
        this.readTopLevelDomainScripts(
            scriptInfo,
            existingScriptInfo,
        );
     ...
     */
    private readSpecialEntryScripts(
            ...
            for (const entry of entries) {
                if (
                    ...
                    const existingDetail =
                        existingScriptInfo?.[domain]?.[
                            scriptProjectName
                        ];
                    const scriptDetail = this.readScriptDetail(
                        entryPath,
                        scriptProjectName,
                        existingDetail,
                    );
                    if (scriptDetail) {
                        scriptInfo[domain][scriptProjectName] =
                            scriptDetail;
                    }
                }
            }
     ...
     * 读取顶级域名下的脚本
     */
    private readTopLevelDomainScripts(
        scriptInfo: ScriptInfo,
        existingScriptInfo?: ScriptInfo,
    ): void {
        ...
        const entries = fs.readdirSync(this.srcPath, {
            withFileTypes: true,
        });

        // 获取所有特殊入口的目录名集合
        const specialEntryNames = new Set(
            SPECIAL_ENTRY_CONFIGS.map((config) => config.entry),
        );
        ...
        for (const entry of entries) {
            ...
            if (
                !entry.isDirectory() ||
                entry.name.startsWith('.') ||
                specialEntryNames.has(entry.name)
            ) {
                continue;
            }
            ...
            this.readSecondLevelDomainScripts(
                scriptInfo,
                topLevelPath,
                topLevelDomain,
                existingScriptInfo,
    ...
    private readSecondLevelDomainScripts(
        ...
        const entries = fs.readdirSync(topLevelPath, {
            withFileTypes: true,
        });
        ...
        for (const entry of entries) {
            ...
            const secondLevelPath = path.join(
                topLevelPath,
                secondLevelDomain,
            );
            ...
            // 读取二级域名下的所有脚本项目
            const scriptEntries = fs.readdirSync(secondLevelPath, {
                withFileTypes: true,
            });
            for (const scriptEntry of scriptEntries) {
                // 跳过以 . 开头的目录
                if (
                    !scriptEntry.isDirectory() ||
                    scriptEntry.name.startsWith('.')
                ) {
                    continue;
                }
    ...
    private readScriptDetail(
        ...
        const userScriptFilepath =
            existingDetail?.userScriptFilepath ||
            `backup/${scriptProjectName}.user.js`;
        const userJsPath = path.join(scriptPath, userScriptFilepath);

        // 如果脚本文件不存在，创建空对象让用户填写
        if (!fs.existsSync(userJsPath)) {
            console.warn(
                `[警告] 未找到脚本文件: ${userJsPath}，将创建空对象`,
            );
            return {
                name: '',
                description: '',
                ...
                downloadUrlFromGithub:
                    existingDetail?.downloadUrlFromGithub || '',
                downloadUrlFromScripCat:
                    existingDetail?.downloadUrlFromScripCat ?? '',
                downloadUrlFromGreasyFork:
                    existingDetail?.downloadUrlFromGreasyFork ?? '',
                autoUpdate: existingDetail?.autoUpdate ?? true,
                getLastUpdateWay:
                    existingDetail?.getLastUpdateWay ?? 'file',
            };
        }

        try {
            const metadata = this.parseUserScriptMetadata(userJsPath);
            const getLastUpdateWay =
                existingDetail?.getLastUpdateWay ?? 'file';
    ...
    write(scriptInfo: ScriptInfo): void {
        ...
        for (const domain in scriptInfo) {
            ...
            for (const scriptProjectName in scriptInfo[domain]) {
                const newDetail =
                    scriptInfo[domain][scriptProjectName];
                const existingDetail =
                    existingScriptInfo[domain]?.[scriptProjectName];

                // 确定 userScriptFilepath：优先保留已有的，否则使用新的
                const userScriptFilepath =
                    existingDetail?.userScriptFilepath ||
                    newDetail.userScriptFilepath;
                ...
                if (existingDetail) {
                        ...
                        mergedScriptInfo[domain][scriptProjectName] =
                            {
                                ...newDetail,
                                private:
                                    existingDetail.private ?? false,
                                archive:
                                    existingDetail.archive ?? false,
                                autoUpdate:
                                    existingDetail.autoUpdate ?? true,
                                userScriptFilepath:
                                    userScriptFilepath,
                                downloadUrlFromScripCat:
                                    existingDetail.downloadUrlFromScripCat ??
                    ...
                    mergedScriptInfo[domain][scriptProjectName] = {
                        ...
                        downloadUrlFromScripCat: '',
                        downloadUrlFromGreasyFork: '',
                        downloadUrlFromGithub: this.generateGithubUrl(
                            domain,
                            scriptProjectName,
                            userScriptFilepath,
                        ),
        ...
        fs.writeFileSync(
            this.scriptJsonPath,
            JSON.stringify(mergedScriptInfo, null, '\t'),
            'utf8',
        );
    ...
    private generateGithubUrl(
        ...
        // 查找是否为特殊入口配置的域名
        const specialConfig = SPECIAL_ENTRY_CONFIGS.find(
...
class ReadmeFileSystem {
    ...
    static write(scriptInfo: ScriptInfo, isPrivate: boolean): void {
        ...
        // 筛选对应权限的脚本
        const filteredScriptInfo = this.filterScriptsByPrivacy(
            scriptInfo,
            isPrivate,
        );
        ...
        console.log(
            `[成功] 已生成 ${isPrivate ? '私有' : '公有'} README: ${readmeFilepath}`,
        );
    }
...
function main(): void {
    ...
    console.log(
        `发现 ${totalScripts} 个脚本，分布在 ${Object.keys(scriptInfo).length} 个域名下`,
    );
