...
(function () {
    ...
    const registerButton = (clickCallback) => {
        const subGroupNodeList =
            document.querySelectorAll('.subgroup-text');
    ...
    const parseAnimationData = (magnetContainer) => {
        const animationItemList = Array.from(
            magnetContainer.querySelectorAll('tbody > tr'),
        );
        const animationInfoList = animationItemList.map((trNode) => {
            ...
            if (!fileNameNode || !fileSizeNode || !updateTimeNode)
                return void 0;
            const fileName =
                fileNameNode.querySelector('a.magnet-link-wrap')
                    ?.textContent || '';
            ...
            const hasLanguage = Object.entries(languageMapper).find(
                ([key]) => upperFileName.includes(key),
            );
            ...
            const hasQuality = Object.entries(qualityMapper).find(
                ([key]) => upperFileName.includes(key),
            );
            ...
            const hasFormat = Object.entries(formatMapper).find(
                ([key]) => upperFileName.includes(key),
            );
            const format = hasFormat ? hasFormat[1] : 'UNKNOWN';
            const magnetLinkNode =
                fileNameNode.querySelector('a.magnet-link');
            if (!magnetLinkNode) return void 0;
            const magnetLink =
                magnetLinkNode.dataset.clipboardText || '';
            return {
                fileName,
                magnetLink,
                language,
        ...
        const sortedAnimationInfoList = animationInfoList.toSorted(
            (a, b) => {
                const parseUpdateTimeWeight = (updateTime) => {
                    const UPDATE_TIME_WEIGHT = 1e12;
                    return (
                        Date.parse(updateTime) / UPDATE_TIME_WEIGHT
                    );
                };
                ...
                const parseQualityWeight = (quality) => {
                    ...
                    return (
                        qualityWeightMapper[quality] * QUALITY_WEIGHT
                    );
                };
                const aWeight =
                    parseLanguageWeight(a.language) +
                    parseQualityWeight(a.quality) +
                    parseUpdateTimeWeight(a.updateTime);
                const bWeight =
                    parseLanguageWeight(b.language) +
...
`;
    ...
    (async () => {
        ...
        const animationName =
            document.querySelector('.bangumi-title')?.textContent;
        if (!animationName) return;
        document
            .querySelectorAll(
                'a.episode-expand.js-expand-episode:not([style="display: none;"])',
            )
            .forEach((item) => item.click());
        registerButton((subGroupNode) => {
            const subGroupNameNode = subGroupNode.firstElementChild;
            if (!subGroupNameNode) return;
            const subGroupName = subGroupNameNode.classList.contains(
                'dropdown',
            )
                ? (
                      subGroupNameNode.querySelector(
                          '.dropdown-toggle',
                      )?.textContent || ''
                  ).trim()
            ...
            const animationInfoList =
                parseAnimationData(tableContainer);
            const animationDataHeader = `# ${animationName}
> - \u5B57\u5E55\u7EC4: ${subGroupName}
> - \u756A\u5267\u94FE\u63A5: ${document.URL}`;
            const animationDataContent =
                stringifyAnimationData(animationInfoList);
            downloadTextFile(
                `${animationName.trim()}-${subGroupName.trim()}.md`,
                `${animationDataHeader}
