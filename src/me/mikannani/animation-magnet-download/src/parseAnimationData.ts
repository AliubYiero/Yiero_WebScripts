...
export const parseAnimationData = (
    magnetContainer: HTMLTableElement,
) => {
    const animationItemList = Array.from(
        magnetContainer.querySelectorAll<HTMLElement>('tbody > tr'),
    );
    const animationInfoList: IAnimationInfo[] = animationItemList.map(
        (trNode) => {
            ...
            const language: ILanguage = hasLanguage
                ? hasLanguage[1]
                : 'UNKNOWN';
            ...
            const quality: IQuality = hasQuality
                ? hasQuality[1]
                : 'UNKNOWN';
            ...
            const format: IFormat = hasFormat
                ? hasFormat[1]
                : 'UNKNOWN';

            const magnetLinkNode =
                fileNameNode.querySelector<HTMLElement>(
                    'a.magnet-link',
                );
    ...
    const sortedAnimationInfoList = animationInfoList.toSorted(
        (a, b) => {
            ...
            const parseLanguageWeight = (language: ILanguage) => {
                ...
                const languageWeightMapper: Record<
                    ILanguage,
                    number
                > = {
                    简体: 1,
                ...
                return (
                    languageWeightMapper[language] * LANGUAGE_WEIGHT
                );
            };
