...
(function() {
  ...
  */
  ...
  const repeatTimeStore = new GmStorage("\u5237\u5C4F\u8BBE\u7F6E.repeat", 0);
  const repeatMinuteStore = new GmStorage("\u5237\u5C4F\u8BBE\u7F6E.minute", 0);
  const showLogStore = new GmStorage("\u5237\u5C4F\u8BBE\u7F6E.showLog", false);
  ...
  function handleVideoDanmakuMutation(records) {
    ...
    for (const record of records) {
      ...
      if (!(target == null ? void 0 : target.classList.contains("bili-danmaku-x-show")) || target.classList.contains("hide")) {
        continue;
      }
  ...
  async function initVideoDanmakuObserver() {
    ...
    videoDanmakuContainer.querySelectorAll(".bili-danmaku-x-dm").forEach(bindContentObserver);
