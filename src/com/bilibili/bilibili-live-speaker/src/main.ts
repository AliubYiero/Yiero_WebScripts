...
import { UnicycleConfig, UnicycleConfigDialog } from './utils/Dialog.ts';
...
const main = async () => {
	...
	let timer = 0;
	const sendDanmaku = ( config: UnicycleConfig, maxContentLength: number = 40 ) => {
		danmaku.send( config.text.repeat( getRandomInt( config.repeatCount.min, config.repeatCount.max ) ).slice( 0, maxContentLength ) );
		const nextSendTime = getRandomInt( config.interval.min, config.interval.max );
		...
		.createToggle( {
			active: {
				...
				onClick() {
					...
					const contentLengthElement = document.querySelector<HTMLElement>( '.input-limit-hint.p-absolute' );
					if ( contentLengthElement ) {
						const matches = contentLengthElement.innerText.match( /(?<=\/)\d+/ );
					...
					timer = window.setTimeout(
						() => sendDanmaku( config, maxContentLength ),
						getRandomInt( config.interval.min, config.interval.max ),
					);
				},
			},
			inactive: {
