...
export class UnicycleConfigDialog {
	 ...
	 * 显示错误信息并标红输入框
	 */
	private showError(input: HTMLElement | null, errorEl: HTMLElement | null, message: string): void {
	 ...
	 * 清除单个输入框的错误状态
	 */
	private clearError(input: HTMLElement | null, errorEl: HTMLElement | null): void {
	...
	private bindEvents(): void {
		...
		const textInput = this.shadow.getElementById('config-text') as HTMLInputElement;
		const charCountDisplay = this.shadow.getElementById('char-count');
		const repeatMinInput = this.shadow.getElementById('config-repeat-min') as HTMLInputElement;
		const repeatMaxInput = this.shadow.getElementById('config-repeat-max') as HTMLInputElement;
		const minInput = this.shadow.getElementById('config-min') as HTMLInputElement;
		const maxInput = this.shadow.getElementById('config-max') as HTMLInputElement;
		...
		// 输入时自动清除对应错误（提升用户体验）
		repeatMinInput?.addEventListener('input', () => this.clearError(repeatMinInput, repeatError));
		repeatMaxInput?.addEventListener('input', () => this.clearError(repeatMaxInput, repeatError));
		minInput?.addEventListener('input', () => this.clearError(minInput, intervalError));
		maxInput?.addEventListener('input', () => this.clearError(maxInput, intervalError));
