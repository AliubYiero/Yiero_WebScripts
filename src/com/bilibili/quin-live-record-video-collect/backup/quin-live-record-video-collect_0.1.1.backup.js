...
(function () {
    ...
    const getBvId = (url) => {
        const bvId = (url.match(new RegExp('(?<=\\/)BV1[^/]+')) ||
            [])[0];
        return bvId;
    };
    ...
    parserMapper.set('245335', (container) => {
        ...
        const liveDurationElement = container.querySelector(
            '.bili-cover-card__stats > .bili-cover-card__stat:last-child > span',
        );
        ...
        const liveDate = (title.match(
            /\d{2,4}年\d{1,2}月\d{1,2}日/,
        ) || [])[0];
        if (!liveDate) return null;
        const playGame = title.match(
            new RegExp('(?<=\u300A)[^\u300B]+(?=\u300B)', 'g'),
        );
    ...
    parserMapper.set('1400350754', (container) => {
        ...
        if (!bvId) return null;
        const liveDuration = liveDurationElement.innerText;
        const liveDate = (title.match(/\d{2,4}-\d{1,2}-\d{1,2}/) ||
            [])[0];
        if (!liveDate) return null;
        const playGameMatches =
            title.match(
                /【quin录播】 \d{2,4}-\d{1,2}-\d{1,2} (.*)/,
            ) || [];
        if (!playGameMatches[1]) return null;
        const playGame = playGameMatches[1]
            .split('+')
            .map((str) => str.trim());
        if (!playGame || !playGame[0]) return null;
        return {
    ...
    parserMapper.set('15810', (container) => {
        ...
        if (!liveDurationElement) return null;
        const videoPublishDateElement = container.querySelector(
    ...
    const initPagination = async () => {
        const pagination = await elementWaiter(
            '.vui_pagenation--btns',
        );
        return {
            currentPage: () => {
            ...
            toFirstPage: () => {
                const firstPageButton = pagination.querySelector(
                    '.vui_button:nth-child(2)',
                );
                if (!firstPageButton) {
                    return false;
                }
            ...
            toLastPage: () => {
                const lastPageButton = pagination.querySelector(
                    '.vui_button:nth-last-child(2)',
                );
                if (!lastPageButton) {
                    return false;
                }
            ...
            toPrevPage: () => {
                const prevPageButton = pagination.querySelector(
                    '.vui_button:first-child',
                );
            ...
            hasNextPage: () => {
                ...
                return Boolean(
                    nextPageButton && !nextPageButton.disabled,
                );
            },
        };
    };
    ...
    const main = async () => {
        gmMenuCommand
            .create('\u6293\u53D6\u89C6\u9891', async () => {
                ...
                const liveRecord = liveRecordStore.get();
                const liveRecordMapper = new Map(
                    liveRecord.map((item) => [item.bvId, item]),
                );
                const container = await elementWaiter(
                    '.video-body > .video-list',
                );
                ...
                while (pagination.hasNextPage()) {
                    for (const item of container.querySelectorAll(
                        ...
                        const uploaderUid = (location.pathname.match(
                            new RegExp('(?<=\\/)\\d+'),
                        ) || [])[0];
                        if (!uploaderUid) {
                            continue;
                        }
                        const info = handleCollectVideo(
                            uploaderUid,
                            item,
                        );
                        if (!info) {
                            console.warn(
                                '[collect-video] \u65E0\u6CD5\u83B7\u53D6\u89C6\u9891\u4FE1\u606F',
                                item,
                            );
                            continue;
                        }
                        ...
                        console.info(
                            '[collect-video] \u83B7\u53D6\u89C6\u9891\u4FE1\u606F',
                            info,
                        );
                        liveRecordMapper.set(info.bvId, info);
                    }
                    liveRecordStore.set(
                        Array.from(liveRecordMapper.values()),
                    );
                    console.info(
                        '[collect-video] \u4FDD\u5B58\u5F53\u524D\u9875\u76F4\u64AD\u8BB0\u5F55',
                    );
            ...
            .create('\u751F\u6210\u65E5\u5FD7(json)', () => {
                ...
                gmDownload.text(
                    JSON.stringify(flatLiveRecord, null, 2),
                    'Quin\u76F4\u64AD\u8BB0\u5F55.json',
                    'application/json',
                );
            })
