...
export const renderSingleUpButton = async () => {
	...
	try {
		...
		const upLinkContainer = container.querySelector<HTMLAnchorElement>( '.up-avatar' );
		if ( !upLinkContainer ) {
			return uidList;
		}
		...
		const upLinkContainerList = Array.from( container.querySelectorAll<HTMLAnchorElement>( '.avatar' ) );
		const list = upLinkContainerList
			.reduce<number[]>( ( list, element ) => {
