class SortButton extends HTMLElement {
    ...
    private updateCountdownDisplay(): void {
        const loadingButton =
            this.shadowRoot?.querySelector('.loading-button');
        if (loadingButton) {
    ...
    private addEventListeners(): void {
        const ascendButton = this.shadowRoot?.querySelector(
            '.ascend-sort-button',
        );
        const descendButton = this.shadowRoot?.querySelector(
            '.descend-sort-button',
        );
 ...
 * 初始化 SortButton
 */
export const initSortButton = (container: HTMLElement) => {
    // 注册 WebComponent
