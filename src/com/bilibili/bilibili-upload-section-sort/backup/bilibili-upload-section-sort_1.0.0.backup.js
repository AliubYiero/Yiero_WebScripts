...
(function () {
    ...
    function Message(options) {
        ...
        const typeConfig =
            messageTypes[detail.type] || messageTypes.info;
        messageEl.setAttribute(
            ...
            `
                ...
                cursor: pointer;
                ${Object.entries(
                    messagePositions[detail.position || 'top'],
                )
                    .map(([k, v]) => `${k}: ${v};`)
                    .join(' ')}
            `,
    ...
    class SortButton extends HTMLElement {
        ...
        addEventListeners() {
            ...
            const ascendButton =
                (_a = this.shadowRoot) == null
                    ? void 0
                    : _a.querySelector('.ascend-sort-button');
            const descendButton =
                (_b = this.shadowRoot) == null
                    ? void 0
                    : _b.querySelector('.descend-sort-button');
            if (ascendButton) {
                ascendButton.addEventListener(
                    'click',
                    this.handleAscendSort.bind(this),
                );
            }
            if (descendButton) {
                descendButton.addEventListener(
                    'click',
                    this.handleDescendSort.bind(this),
                );
            }
        }
        // 移除事件监听器
        removeEventListeners() {
            ...
            const descendButton =
                (_b = this.shadowRoot) == null
                    ...
                    : _b.querySelector('.descend-sort-button');
            if (ascendButton) {
                ascendButton.removeEventListener(
                    'click',
                    this.handleAscendSort.bind(this),
                );
            }
            if (descendButton) {
                descendButton.removeEventListener(
                    'click',
                    this.handleDescendSort.bind(this),
                );
            }
    ...
    const main = async () => {
        ...
        const handleSeasonData = async (response) => {
            ...
            const processEpisode = async (episode) => {
                ...
                if (cachedTime) {
                    videoPublishInfoList.push({
                        id,
                        publishTime: cachedTime,
                    });
                    return;
                }
