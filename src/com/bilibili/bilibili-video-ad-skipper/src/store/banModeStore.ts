 ...
 */
export const banModeStore = new GmStorage<'白名单' | '黑名单'>( '屏蔽设置.banMode', '黑名单' );
const whitelistUpStore = new StringListStore( '屏蔽设置.whitelist', new Set<string>() );
const blacklistUpStore = new StringListStore( '屏蔽设置.blacklist', new Set<string>() );
export const currentBanListStore = banModeStore.get() === '白名单'
	? whitelistUpStore
	: blacklistUpStore;
