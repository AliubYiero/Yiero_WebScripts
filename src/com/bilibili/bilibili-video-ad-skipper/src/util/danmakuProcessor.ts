 ...
 */
const AD_KEYWORDS = [
	'恰饭', '金主', '商单', '植入', '赞助', '合作', '广告',
	'链接', '口令', '优惠', '折扣', '下载', '购买', '点击',
	'感谢', '冠名', '支持', 'APP', '产品', '推荐',
];
...
export const aggregateDanmaku = (
	...
	for ( let i = 0; i < numWindows; i++ ) {
		...
		windowDanmaku.forEach( d => {
			messageCount.set( d.text, ( messageCount.get( d.text ) || 0 ) + 1 );
		} );

		// 分离高频弹幕(复读≥3次)和其他弹幕(出现1次)
		const highFreqMessages: Array<{ text: string; count: number }> = [];
		...
		// 提取示例弹幕（前3条高频弹幕文本）
		const sampleTexts = highFreqMessages.slice( 0, 3 ).map( h => h.text );
...
export const detectDensitySpike = (
	...
	if ( aggregatedGroups.length < 3 ) {
		return aggregatedGroups.map( g => ( { ...g, densitySpike: 'normal' as const } ) );
	}

	// 计算平均弹幕密度
	const avgCount = aggregatedGroups.reduce( ( sum, g ) => sum + g.count, 0 ) / aggregatedGroups.length;
...
export const formatDanmakuGroupsToXML = (
	...
	const blockElements = groups.map( group => {
		const highFreqText = group.highFreqMessages
			.map( h => `${ h.text }(x${ h.count })` )
			.join( ' | ' ) || '无';

		const othersText = group.otherMessages.join( ' | ' ) || '无';
