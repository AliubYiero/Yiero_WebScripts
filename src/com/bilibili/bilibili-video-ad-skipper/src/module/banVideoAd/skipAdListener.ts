 ...
 * 检查时间点是否在广告时间段内
 */
function isTimeInAd( time: number, adTimes: AdTime[] ): AdTime | undefined {
 ...
 */
export const skipAdListener = async (
	adTimes: AdTime[],
) => {
	const video = await elementWaiter<HTMLVideoElement>( '[aria-label="哔哩哔哩播放器"] video' );
