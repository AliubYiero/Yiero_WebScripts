...
export const extractGrantPlugin = () => {
    return {
        ...
        generateBundle(_: any, bundle: OutputBundle) {
            ...
            const gmCatRegex =
                /(GM_[a-zA-Z0-9_]+|CAT_[a-zA-Z0-9_]+|window.onurlchange|window.close|window.focus)/g;
            // 已经声明的 grant 函数
            const grantFunctionNameList = Object.values(UserScript)
                .filter(([key]) => key === 'grant')
                .map(([_, value]) => value);
            const grantFunctionNameSet = new Set<string>(
                grantFunctionNameList,
            );
            ...
            for (const fileName in bundle) {
                ...
                for (let matchAllElement of chunk.code.matchAll(
                    gmCatRegex,
                )) {
 ...
 * 插件 3: 输出 banner
 */
export const bannerPlugin = (content: string | (() => string)) => {
    return {
    ...
    return {
        ...
        generateBundle(_: any, bundle: OutputBundle) {
            for (const fileName in bundle) {
                ...
                mkdirSync(join(__dirname, 'dist', version), {
                    recursive: true,
                });
