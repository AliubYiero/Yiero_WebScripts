...
export const parseUserScript = (
    ...
    // 记录只需要 key 值的属性
    const booleanKeySet: Set<IUserScriptKey> = new Set([
        'background',
        'early-start',
        'noframes',
    ]);
    // 过滤掉空属性
    const filterUserScript = userScript.filter(
        ([key, value]) =>
            booleanKeySet.has(<IUserScriptKey>key) || value,
    );

    // 将 Entry 数组形式的 UserScript 转换成对象
    const userScriptObject: Record<string, string> =
        Object.fromEntries(filterUserScript);

    // 如果是 开发环境, 在 UserScript 中加入 require 自我引用
    const outputFileName = userScriptObject.name + '.dev.js';
    isDevelopment &&
        filterUserScript.push([
            'require',
...
export const validateUserScript = (
    ...
    if (typeof mapper !== 'object') {
        throw new Error(
            '缺失脚本信息: name, version, description, author',
        );
    }
