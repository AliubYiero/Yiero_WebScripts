...
import { videoSignProcessingQueue } from '../../videoSignProcessingQueue/videoSignProcessingQueue.ts';
import { elementWaiter } from '@yiero/gmlib';
import { createMutationObserver } from '../../../util/createMutationObserver.ts';
...
export const baseParser = (
    ...
    // 获取视频链接
    const videoLink = container.querySelector<HTMLAnchorElement>(
        selectorList.videoLink,
    );
    ...
    for (let loadedContainer of loadedContainerList) {
        ...
        if (
            ...
            const loadedItemList = Array.from(
                loadedContainer.querySelectorAll<HTMLElement>(
                    selectorList.item,
                ),
            ).map(parser);
...
export class ObserverList {
    // 通用视频元素标记观察者列表
    private static observerSet: Set<MutationObserver> =
        new Set<MutationObserver>();
