        ...
        async writeBundle(_: any, bundle: Record<string, Object>) {
            for (const file of Object.entries(bundle)) {
                ...
                try {
                    ...
                    const GrantFunctionList: string[] = [
                        'GM_addStyle',
                        'GM_addElement',
                        'GM_getResourceText',
                        'GM_getResourceURL',
                    ...
                    GrantFunctionList.forEach((grantFunctionName) => {
                        if (
                            data.includes(grantFunctionName) &&
                            !UserScript.find(
                                ([key, grantFunction]) =>
                                    key === 'grant' &&
                                    grantFunction ===
...
const stringifyBannerContent = (
     ...
     * */
    // 去除空属性 (除了 background)
    const userScript = UserScript.filter(
        (info) => info[1].trim() || info[0] === 'background',
    );

    // 获取所有 key 中的最大长度
    const UserScriptKeyMaxLength: number = userScript.reduce<number>(
        (maxLength, item) => Math.max(item[0].length, maxLength),
        0,
    );
    // 如果是 开发环境, 在 UserScript 中加入 require 自我引用
    const selfRequire = [
        'require',
        pathToFileURL(resolve(__dirname, 'dist', outputFileName))
            .href,
    ] as IUserScript;
    ...
    bannerContentList.push(
        ...[
            ...
            userScript
                .map(([key, value]) => {
                    const formatKey = key.padEnd(
                        UserScriptKeyMaxLength,
                        ' ',
                    );
...
export default defineConfig(({ mode }) => {
     ...
     * 将 UserScript 转换成对象
     */
    const userScript: Record<IUserScriptKey, string> =
        UserScript.reduce(
    ...
    if (
        !(
            userScript.match ||
            userScript.crontab ||
            userScript.background
        )
    ) {
        throw new Error(
            `缺失脚本信息: match, crontab 或者 background`,
        );
    }
    ...
    const backupProductionFilepath = resolve(
        __dirname,
        'dist',
        userScript.version,
        outputFileName,
    ...
    if (isProduction && existsSync(backupProductionFilepath)) {
        throw new Error(
            `存在文件 [${backupProductionFilepath}], 禁止覆盖版本备份文件...`,
        );
    }
