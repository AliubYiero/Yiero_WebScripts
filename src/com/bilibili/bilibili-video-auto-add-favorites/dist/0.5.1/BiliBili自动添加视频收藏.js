...
const getFavouriteTitle = () =>
    GMStorageExtra.getItem(
        favouriteTitleConfig.key,
        favouriteTitleConfig.title,
    );
...
const hasFavouriteTitle = () =>
    GMStorageExtra.hasItem(favouriteTitleConfig.key);
...
const api_createFavorites = (favTitle) =>
    xhrRequest('/x/v3/fav/folder/add', 'POST', {
        title: favTitle,
        privacy: 1,
        csrf: requestConfig.csrf,
    });
...
const api_sortFavorites = async (favoriteIdList) =>
    xhrRequest('/x/v3/fav/folder/sort', 'POST', {
...
const checkFavoriteIsFull = (favoriteInfo) =>
    favoriteInfo.media_count === config.MAX_MEDIA_COUNT;
...
const addVideoToFavorite = async (videoId, latestFavorite) => {
    ...
    const successfullyAdd =
        (res == null ? void 0 : res.success_num) === 0;
    if (!successfullyAdd) {
        return;
    }
    (() => {})(
...
const sortOlderFavoritesToLast = async (userUid) => {
    ...
    let readFavouriteList = favoriteList.filter((favoriteInfo) =>
        favoriteInfo.title.startsWith(favoriteTitle),
    );
    let otherFavouriteList = favoriteList.filter(
        (favoriteInfo) =>
            !favoriteInfo.title.startsWith(favoriteTitle),
    );
    readFavouriteList.sort((a, b) => {
...
const api_isFavorVideo = () =>
    request('/x/v2/fav/video/favoured', 'GET', {
        aid: getVideoAvId(),
    }).then((res) => {
        if (res.code !== 0) {
            throw new Error(res.message);
...
const autoAddVideoToFavourites = async () => {
    ...
    const latestFavourite = readFavouriteList[0];
    const isFullInFavorite = checkFavoriteIsFull(
        readFavouriteList[0],
    );
    if (!isFullInFavorite) {
    ...
    if (isFullInFavorite) {
        const favoriteTitle = getFavouriteTitle();
        const latestFavouriteId = Number(
            latestFavourite.title.slice(favoriteTitle.length),
        );
        await createNewFavorite(
            `${favoriteTitle}${latestFavouriteId + 1}`,
        );
        await sleep(1);
    ...
    const favButtonSelector =
        '.video-fav.video-toolbar-left-item:not(.on)';
    const favButtonDom = await getElement(
        document,
        favButtonSelector,
    );
    if (favButtonDom) {
