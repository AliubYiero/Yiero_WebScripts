...
describe('getVideoSubtitlesList', () => {
    ...
    test('当分P不存在时应该抛出错误', async () => {
        ...
        await expect(
            getVideoSubtitlesList('BV1xx411c7mD', 99),
        ).rejects.toThrow('分P 99 不存在，视频共 2P');
    });
    ...
    test('当视频没有分P信息时应该抛出错误', async () => {
        ...
        vi.mocked(
            xhrRequest.getWithCredentials,
        ).mockResolvedValueOnce(noPagesResponse);

        await expect(
            getVideoSubtitlesList('BV1xx411c7mD'),
        ).rejects.toThrow('视频 BV1xx411c7mD 没有分P信息');
    });
