...
const handleParseDanmakuTime = (
    ...
    const dataset = container.dataset as Record<
        'timestamp' | 'ts',
        string
    >;
    ...
    // 获取弹幕文本容器
    const danmakuContentContainer =
        container.querySelector<HTMLElement>('.danmaku-item-right');
    ...
    const sendTime = ts === 0 ? timestamp : ts * 1000;
...
export const appendDanmakuSendTime = (
    ...
    const isLoadChatStyleScript = Boolean(
        document.head.querySelector('style.bilibili-live-chat-style'),
    );
    // 监听页面弹幕加载
