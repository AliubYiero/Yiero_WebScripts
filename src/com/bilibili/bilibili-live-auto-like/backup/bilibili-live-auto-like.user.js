...
(function () {
    ...
    const handleLike = (
        ...
        const currentRoomMaxLikeCounter =
            maxLikeCounter.room[String(roomId)] || 0;
        if (currentRoomMaxLikeCounter > MAX_LIKE_NUMBER) {
            return;
        }
        if (
            propSelectorList.some((selector) =>
        ...
        gmMenuCommand.batch(() => {
            gmMenuCommand
                .reset()
                .create(
                    `\u623F\u95F4 ${roomId} \u70B9\u8D5E\u6570: ${nextRoomMaxLikeCounter}`,
                    () => {},
                )
                .create(
                    `${todayDate} \u70B9\u8D5E\u603B\u6570: ${nextTotalLikeCounter}`,
                    () => {},
                );
        });
        handleTimeoutLike();
    ...
    const main = async () => {
        ...
        if (!liveRoomContainer) {
            console.info(
                `[Bilibili Live Auto Like] \u52A0\u8F7D\u6D3B\u52A8\u9875\u9762...`,
            );
            return;
        }
        ...
        if (isOnlyLikeFollow) {
            const isFollowing =
                !document.querySelector('.not-yet-follow');
            if (!isFollowing) return;
        }
        const roomId = getRoomId();
        if (!roomId) {
            console.error(
                `[Bilibili Live Auto Like] \u9519\u8BEF\u83B7\u53D6\u623F\u95F4\u53F7, \u83B7\u5F97\u7A7A\u5185\u5BB9`,
            );
            return;
        }
        ...
        gmMenuCommand.batch(() => {
            gmMenuCommand
                .create(
                    `\u623F\u95F4 ${roomId} \u70B9\u8D5E\u6570: ${currentRoomMaxLikeCounter}`,
                    () => {},
                )
                .create(
                    `${currentTodayDate} \u70B9\u8D5E\u603B\u6570: ${currentTodayLikeCounter}`,
                    () => {},
                );
        });
