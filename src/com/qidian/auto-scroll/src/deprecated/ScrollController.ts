...
export class ScrollController {
     ...
     */
    async init() {
        let { current: elementList, nextElement } =
            this.getViewportElements();
        // 计算文本长度
        const contentLength = elementList.reduce(
        ...
        const { scrollTime } = this.getScrollLength(
            contentLength,
            this.viewHeight,
        );
        ...
        while (nextElement) {
            const { height, length } =
                this.getElementInfo(nextElement);
            const { scrollCount, scrollHeightPerCount } =
            ...
            nextElement = this.container.querySelector(
                `${this.childSelector}[data-sign="true"] + ${this.childSelector}:not([data-sign="true"])`,
            );
        }
    }
    ...
    private getScrollLength(
        ...
        // 需要滚动的次数
        const scrollCount = Math.round(
            this.scrollCountPerSecond * scrollTime,
        );
        // 每次滚动的高度
        const scrollHeightPerCount = Math.round(
            contentHeight / scrollCount,
        );
        return {
            scrollTime,
            scrollCount,
    ...
    private getViewportElements() {
        ...
        for (const el of elements) {
            ...
            const isInCurrent =
                rect.bottom > 0 && rect.top < this.viewHeight;
