...
const calculatePriceSet = () => {
    ...
    const { 背包: bagPriceList = [], 胸挂: necklacePriceList = [] } =
    ...
    for (let necklacePriceItem of necklacePriceList) {
        priceList.push({
            ...
            singleCapacityPrice:
                necklacePriceItem.price / necklacePriceItem.capacity,
        });
    }
...
const main = async () => {
    gmMenuCommand
        ...
        .create('下载背包最佳性价比(csv)', () => {
            ...
            const bestList: BackpackPriceItem[] =
                computedBestPrice(priceList);
