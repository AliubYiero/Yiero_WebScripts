...
export const appendBackpackCapacityUi = (container: HTMLElement) => {
    ...
    const priceColumn =
        headerContainer.querySelector<HTMLElement>('.price-column');
    if (!priceColumn) {
        return false;
    }
    ...
    rows.forEach((row) => {
        const itemCell = row.querySelector<HTMLElement>(
            '.item-cell .item-name',
        );
        if (!itemCell) return;
        ...
        // 读取 type
        const type = new URLSearchParams(location.search).get('n') as
            | '胸挂'
            | '背包';
        if (!type) return;

        const priceCell =
            row.querySelector<HTMLElement>('.price-cell');
...
"/>`;
        ...
        // 添加 dataset
        const itemPriceCell = row.querySelector<HTMLElement>(
            '.price-cell:has(.icon-gold)',
        );
        const standardItemPriceCell = row.querySelector<HTMLElement>(
