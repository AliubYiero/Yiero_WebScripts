...
(function () {
    ...
    const backpackCapacityStore = new GmStorage(
        'backpackCapacity',
        backpackCapacities,
    );
    const appendBackpackCapacityUi = (container) => {
        ...
        const priceColumn =
            headerContainer.querySelector('.price-column');
        if (!priceColumn) {
            return false;
        }
        const backpackCapacityColumn = document.createElement('th');
        backpackCapacityColumn.classList.add(
            'backpack-capacity-column',
        );
        backpackCapacityColumn.textContent =
            '\u80CC\u5305\u5BB9\u91CF';
        headerContainer.insertBefore(
            backpackCapacityColumn,
            priceColumn,
        );
        ...
        rows.forEach((row) => {
            ...
            backpackCapacityCell.classList.add(
                'backpack-capacity-cell',
            );
...
"/>`;
            ...
            const itemPriceCell = row.querySelector(
                '.price-cell:has(.icon-gold)',
            );
            const standardItemPriceCell = row.querySelector(
                '.price-cell:has(.icon-gold):nth-child(6)',
            );
            if (!itemPriceCell || !standardItemPriceCell)
                return false;
            const itemPrice = Number(
                itemPriceCell.innerText.replace(/,\s?/, ''),
            );
            const standardItemPrice =
                itemPrice +
                -1 *
                    Number(
                        standardItemPriceCell.innerText.replace(
                            /,\s?/,
                            '',
                        ),
                    );
        ...
        container.addEventListener('input', (e) => {
            ...
            if (!input.classList.contains('backpack-capacity-input'))
                return;
    ...
    const clonePriceData = (container) => {
        ...
        const rows = container.querySelectorAll('tbody > .table-row');
        rows.forEach((row) => {
            const { name, type, price, capacity, standardPrice } =
                row.dataset;
            if (
                !name ||
                !type ||
    ...
    const calculatePriceSet = () => {
        ...
        for (const bagPriceItem of bagPriceList) {
            priceList.push({
                ...
                singleCapacityPrice:
                    bagPriceItem.price / bagPriceItem.capacity,
            });
            for (let necklacePriceItem of necklacePriceList) {
                priceList.push({
                    ...
                    necklaceStandardPrice:
                        necklacePriceItem.standardPrice,
                    totalCapacity:
                        bagPriceItem.capacity +
                        necklacePriceItem.capacity,
                    totalPrice:
                        bagPriceItem.price + necklacePriceItem.price,
                    totalStandardPrice:
                        bagPriceItem.standardPrice +
                        necklacePriceItem.standardPrice,
                    singleCapacityPrice:
                        (bagPriceItem.price +
                            necklacePriceItem.price) /
                        (bagPriceItem.capacity +
                            necklacePriceItem.capacity),
                });
            }
...
`;
    ...
    const computedBestPrice = (priceList) => {
        return Object.values(
            Object.groupBy(priceList, (item) => item.totalCapacity),
        )
            .map((list) => {
                if (!list) return;
    ...
    const main = async () => {
        ...
        const isBackpackPage = Boolean(
            type && ['\u80F8\u6302', '\u80CC\u5305'].includes(type),
        );
        if (!isBackpackPage) return;
        const container = await elementWaiter('.modern-table', {
            delayPerSecond: 0.05,
        });
        if (!container) return;
