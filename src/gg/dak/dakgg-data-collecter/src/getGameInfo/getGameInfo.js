...
export const getGameInfo = (gameInfoContainer) => {
    ...
    originMatchDataKeyMapper.forEach(([originKey, outputKey]) => {
        // 主视角数据
        gameInfo[outputKey] =
            selfInfo[originKey] == null ? '' : selfInfo[originKey];
        // 队友数据
    ...
    matchDateReplacer((info) => {
        ...
        const selectCharacter =
            characters.find((item) => item.id === info.character) ||
            {};
        ...
        // 潜能信息
        const traitSkills =
            MatchDataMapperStorage.get('trait-skills').traitSkills;
        ...
        // 获取技能数据数组
        const skillInfoList = Array.from(skillIdSet).map((skillId) =>
            skills.find((item) =>
