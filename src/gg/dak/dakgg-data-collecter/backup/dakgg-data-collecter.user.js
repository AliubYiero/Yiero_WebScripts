...
(function () {
    ...
    const freshListenerPushState = async (
        callback,
        delayPerSecond = 1,
    ) => {
        let _pushState = window.history.pushState.bind(
            window.history,
        );
    ...
    const getGameInfo = (gameInfoContainer) => {
        const gameInfo = {
            ...
            teammate: [],
            // 对局Id
            gameId: gameInfoContainer.querySelector(
                '.game-info > .game > .id',
            ).textContent,
        };
        const originMatchData = MatchDataStorage.get(gameInfo.gameId);
        const selfInfo = originMatchData.matches.find(
            (info) => info.nickname === gameInfo.player,
        );
        const teammateInfoList = originMatchData.matches.filter(
            (info) => {
                if (
                    !(
                        info.gameRank === selfInfo.gameRank &&
                        info.nickname !== gameInfo.player
                    )
                ) {
                    return false;
                }
        ...
        const replaceMatchData = (
            selfMatchData,
            teammateListMatchData,
            callback,
        ) => {
        ...
        const matchDateReplacer = replaceMatchData.bind(
            void 0,
            gameInfo,
            gameInfo.teammate,
        );
        originMatchDataKeyMapper.forEach(([originKey, outputKey]) => {
            gameInfo[outputKey] =
                selfInfo[originKey] == null
                    ? ''
                    : selfInfo[originKey];
            teammateInfoList.forEach((teammateInfo, index) => {
                gameInfo.teammate[index][outputKey] =
                    teammateInfo[originKey] == null
                        ? ''
                        : teammateInfo[originKey];
            });
        });
        matchDateReplacer((info) => {
            const seasons =
                MatchDataMapperStorage.get('seasons').seasons;
            const currentSeason = seasons.find(
                (item) => item.isCurrent,
            );
            const selectSeasonKey = new URLSearchParams(
                location.search,
            ).get('season');
            ...
            info.gameMode =
                gameModes.find((item) => item.id === info.gameMode)
                    ?.name || '';
            info.versionMajor = selectSeason.id >= 18 ? 1 : 0;
            info.gameVersion = `v${info.versionMajor}.${info.gameVersionMajor}.${info.gameVersionMinor}`;
            const characters =
                MatchDataMapperStorage.get('characters').characters;
            const selectCharacter =
                characters.find(
                    (item) => item.id === info.character,
                ) || {};
            info.character = selectCharacter?.name || '';
            info.skin =
                selectCharacter.skins?.find(
                    (item) => item.id === info.skin,
                )?.name || '';
            const weapons =
                MatchDataMapperStorage.get('masteries').masteries;
            const selectWeapon =
                weapons.find((item) => item.id === info.weapon) || {};
            info.weapon = selectWeapon?.name || '';
            const tacticalSkills =
                MatchDataMapperStorage.get(
                    'tactical-skills',
                ).tacticalSkills;
            const selectTacticalSkill =
                tacticalSkills.find(
                    (item) => item.id === info.tacticalSkill,
                ) || {};
            ...
            const traitSkillGroups = [
                ...MatchDataMapperStorage.get('trait-skills')
                    .traitSkillGroups,
                { key: 'Cobalt', name: '\u94B4\u534F\u8BAE' },
            ];
            ...
            const traitFirstCore = traitSkills.find(
                (item) => item.id === info.traitFirstCore,
            );
            const traitFirstGroup = traitSkillGroups.find(
                (item) => item.key === traitFirstCore.group,
            );
            const traitFirstGroupName = traitFirstGroup?.name || '';
            const traitFirstSub = info.traitFirstSub.map((id) =>
                traitSkills.find((item) => item.id === id),
            );
            const traitSecondSub = info.traitSecondSub.map((id) =>
                traitSkills.find((item) => item.id === id),
            );
            const traitSecondGroup = traitSkillGroups.find(
                (item) => item.key === traitSecondSub[0].group,
            );
            const traitSecondGroupName = traitSecondGroup?.name || '';
            const stringifyTrait = (groupName, traitInfo) =>
                `${traitInfo.name} (${groupName}, ${traitTypeMapper[traitInfo.type]})`;
            info.traitCore = [
                `\u4E3B\u6F5C\u80FD: ${traitFirstGroupName}`,
                stringifyTrait(traitFirstGroupName, traitFirstCore),
                ...traitFirstSub.map((trait) =>
                    stringifyTrait(traitFirstGroupName, trait),
                ),
            ];
            info.traitSub = [
                `\u526F\u6F5C\u80FD: ${traitSecondGroupName}`,
                ...traitSecondSub.map((trait) =>
                    stringifyTrait(traitSecondGroupName, trait),
                ),
            ];
            delete info.traitSecondSub;
            delete info.traitFirstSub;
            delete info.traitFirstCore;
            const skills = MatchDataMapperStorage.get(
                'skills',
            ).skills.map((skillInfo) => ({
                ...skillInfo,
                id: Math.floor(skillInfo.id / 100),
            }));
            const skillOrderList = Object.values(info.skillOrder).map(
                (skillId) => Math.floor(skillId / 100),
            );
            const skillIdSet = new Set(skillOrderList);
            const skillInfoList = Array.from(skillIdSet).map(
                (skillId) =>
                    skills.find((item) =>
                        item.id.toString().startsWith(skillId),
                    ),
            );
            const skillMapper = skillInfoList.reduce(
                (skillMapper2, skill) => {
                    if (skill) {
            ...
            info.skillOrder = skillOrderList
                .map((skillId) => skillMapper[skillId])
                .join(' -> ');
    ...
    const downloadTextFile = (text, filename) => {
        const blob = new File([text], filename, {
            type: 'text/plain',
        });
    ...
    const clickBtnHandle = (event, gameInfoContainer, e) => {
        ...
        const stringifyGameInfo = JSON.stringify(
            gameInfo,
            void 0,
            '	',
        );
    ...
    const addButton = (gameInfoContainer) => {
        ...
        tabContainer.appendChild(
            createDownloadBtn(gameInfoContainer),
        );
        const gameInfoSectionContainer =
            gameInfoContainer.querySelector(
