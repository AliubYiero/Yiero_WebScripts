...
(function () {
    ...
    const RedeemCode = {
        ...
        action: {
            ...
            isRedeemCode: (message) => {
                ...
                return (
                    /^[a-zA-Z0-9]{5,}$/.test(trimmedMessage) &&
            ...
            addRedeemCode: (redeemCode) => {
                ...
                if (
                    RedeemCode.data.redeemCodeList.includes(
                        redeemCode,
                    )
                )
                    return;
                ...
                if (
                    ...
                    RedeemCodeGrantEvent.send(
                        redeemCode,
                        RedeemCode.data.redeemCodeList.indexOf(
                            redeemCode,
                        ),
                    );
    ...
    const liveListener = async (
        selectorList2,
        handleAddRedeemCode2,
        target,
    ) => {
        const menuTitle =
            target === 'bilibili'
        ...
        gmMenuCommand
            ...
            .render();
        const chatItemContainer = await elementWaiter(
            selectorList2.container,
            {
                timeoutPerSecond: 0,
            },
        );
        const existingMessages = chatItemContainer.querySelectorAll(
            selectorList2.danmakuItem,
        );
        existingMessages.length &&
            existingMessages.forEach(handleAddRedeemCode2);
        ...
        danmakuObserver.observe(chatItemContainer, {
            childList: true,
        });
    };
    const selectorList$1 = {
        container:
            'div:has(> .event.event--message.event-type--message)',
        danmakuItem: '.event.event--message.event-type--message',
    };
    const handleAddRedeemCode$1 = (element) => {
        const danmakuElement = element.querySelector(
