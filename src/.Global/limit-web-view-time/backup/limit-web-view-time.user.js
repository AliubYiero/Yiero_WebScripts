...
(function () {
    ...
    function chain(...funcs) {
        return (...args) => {
            return funcs
                .slice(1)
                .reduce((acc, fn) => fn(acc), funcs[0](...args));
        };
    }
    const UserConfig = {
        '\u914D\u7F6E\u9879': {
    ...
    class GMStorageExtra extends Storage {
        ...
        static getItem(key, defaultValue, group) {
            /* @__PURE__ */ (() => {})(this.createKey(key, group));
            return GM_getValue(
    ...
    const parseUrl = (url) => {
        if (url.startsWith('/') && url.match(/\/[gmyuisd]*$/)) {
            const [, matchRegexString, flag] = url.match(
                /^\/(.*)\/([gmyuisd]*)$/,
            );
    ...
    const findAimWeb = (url) => {
        const limitTimeString = GMStorageExtra.getItem(
            'limitTime',
            '',
        );
        const limitTimeList = parseLimitTimeString(limitTimeString);
        return limitTimeList.find((limitTime) =>
            url.match(limitTime.url),
        );
    };
    const isLimitTime = (aimWeb) => {
        ...
        const currentTime = Date.parse(
            `1970-1-1 ${todayDate.getHours()}:${todayDate.getMinutes()}:${todayDate.getMinutes()}`,
        );
        if (aimWeb.limitWay === LimitWay.limit) {
            const isClose =
                currentTime >= aimWeb.startTime &&
                currentTime <= aimWeb.endTime;
            return isClose;
        }
        const isOpen =
            currentTime >= aimWeb.startTime &&
