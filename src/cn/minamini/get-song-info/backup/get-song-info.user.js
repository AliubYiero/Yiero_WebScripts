...
(function() {
  ...
  const getSongInfo = (element) => {
    ...
    const singerElement = element.querySelector("td:nth-child(3)");
    ...
    let song = "";
    const songChildElement = Array.from(songElement.childNodes).find(
      (item) => item.nodeType === Node.TEXT_NODE && item.textContent && item.textContent.trim()
      // 节点内容不为空白
    );
    if (songChildElement) {
  ...
  const main = async () => {
    gmMenuCommand.create("\u722C\u53D6\u672C\u9875\u4FE1\u606F", handleGetCurrentPageInfo).create("\u83B7\u53D6\u6240\u6709\u6B4C\u66F2\u4FE1\u606F, \u590D\u5236\u5230\u526A\u5207\u677F", () => {
